...
export class PizzariasApplicationService {
  ...
  async adicionaNovaPizzaria(novaPizzariaDto: NovaPizzariaDto): Promise<PizzariaCadastradaDto> {
    const pizzaria = new Pizzaria()
    pizzaria.endereco = novaPizzariaDto.endereco
    pizzaria.nome = novaPizzariaDto.nome
    const pizzariaCadastrada = await this.pizzariasService.adicionaNovaPizzaria(pizzaria)
